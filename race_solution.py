...
class Races:#(s)
    ...
    def class_table_result(self, boat_type, results_string):#(s)
        ...
        #                  CountryNumber-PositionInTable-TotalPoints
        ...
        ########################
        #priorities = [w[0:2] for w in last_race]
        ...
        def resolve_ties(x_count, prob_start, prob_end):
            ...
            # reset problematic range if country in question has a unique point value
            if prob_end == prob_start:
                arr = [country_results[prob_start]]

            else:  # deal with problematic range
        ...
        xx_count = 0
        while i < 10:
            # check problematic element points is the same as current country (ties)
            if country_results[prob_start][2] == country_results[i][2]:  # if so
                # extend problematic range
                prob_end += 1
            else:
    ...
    def class_table_discard_result(self, boat_type, results_string):#(s)
        ...
        def format_output(to_format):
            for wr in range(len(to_format)):
                to_format[wr][1] = "0"+str(wr+1) if wr+1 < 10 else str(
                    wr+1)
                to_format[wr][2] = "0"+str(
                    to_format[wr][2]) if to_format[wr][2] < 10 else str(
                    to_format[wr][2])
            output = ""
        ...
        max_double=[0 for i in range(10)]
        max_single=[0 for i in range(10)]
    ...
    def medal_table_result(self, results_string):#(s)
        ...
        for i in range(10):
            boat_medal_arr=boat_type_medals_per_country(self.class_table_discard_result(i+1,results_string).split(", "), boat_medal_arr)
            
        boat_medal_arr = sorted(boat_medal_arr,key = lambda x : (x[4],x[1],x[2],x[3]), reverse=True)

        return format(boat_medal_arr)
