...
# Creating game window NOT CANVAS
...
def startNewGame():
    ...
    createInGameButtons()
    p1 = Player(500,500,15)

    # Function to set the flag when the up key is pressed
...
#############################################
def inGameMenuPause():
    global btnPauseResume,  btnPauseCheats,btnPauseExit
...
def createInGameButtons():
    global btn_MenuToPause
    ...
    def updatePlayer(self):
        ...
        if not self.onGround:
            if self.consecJumpFrames<self.maxConseqFrames and self.holdingJump:
                ...
                self.consecJumpFrames=5
        if self.y + self.vel < 500:
            canvas.move(self.playerInt,0, self.vel)
            self.y += self.vel
        else:
            # player attaches to ground
            canvas.move(self.playerInt,0,500-self.y)
            self.y+=500-self.y
            self.vel = 0
            self.onGround = True
            self.consecJumpFrames=0
